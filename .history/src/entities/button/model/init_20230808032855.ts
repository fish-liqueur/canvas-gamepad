...
export default function initButton(context: CanvasRenderingContext2D, path: Path2D, canvasSide: number, options: ButtonDefinitionCorner | ButtonDefinitionRadial): void {
    ...
    if (options.icon) {
        context.fillStyle = options.icon.color;
        context.font = `${options.icon.size}px material-icons`;
        context.fillText(options.icon.key, options.icon.x, options.icon.y);
    }
}